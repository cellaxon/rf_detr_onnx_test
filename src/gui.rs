use eframe::egui;
use rf_detr_onnx_test::{detect_objects, Detection};
use std::fs;
            ...
            Ok(image_data) => {
                match detect_objects(&image_data) {
                    Ok((detections, result_image)) => {
                        ...
                        if let Ok(()) = result_image.write_to(&mut std::io::Cursor::new(&mut buffer), image::ImageFormat::Png) {
